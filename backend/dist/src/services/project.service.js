 ...
 */
class ProjectService {
    static transformInputForDatabase(input) {
        return { ...input };
    }
    /**
    ...
    static async create(input) {
        try {
            if (input.isCarousel && input.orderCarousel !== undefined) {
                const orderExists = await project_model_1.ProjectModel.findOne({
                    orderCarousel: input.orderCarousel,
                });
                if (orderExists) {
                    throw new Error(`Order ${input.orderCarousel} is already in use by another project.`);
                }
            }
            const dataForDatabase = this.transformInputForDatabase(input);
            const project = await project_model_1.ProjectModel.create(dataForDatabase);
    ...
    static async update(id, input) {
        ...
        if (input.isCarousel && input.orderCarousel !== undefined) {
            ...
            if (orderExists) {
                throw new Error(`A ordem ${input.orderCarousel} já está em uso por outro projeto.`);
            }
        }
        ...
        const project = await project_model_1.ProjectModel.findByIdAndUpdate(id, dataForDatabase, {
            new: true,
    ...
    static async findAll() {
        const projects = await project_model_1.ProjectModel.find().lean();
   ...
   */
    static async findPaginated(page = 1, limit = 6) {
        ...
        try {
            ...
            const totalCount = await project_model_1.ProjectModel.countDocuments(query);
            // 2. Busca dos projetos com skip e limit
            const projects = await project_model_1.ProjectModel.find(query)
    ...
    static async findBySlug(slug) {
        const project = await project_model_1.ProjectModel.findOne({ slug })
            .populate("team")
            .lean();
        if (!project)
            return null;
    ...
    static async findByStatus(status) {
        const projects = await project_model_1.ProjectModel.find({ status }).lean();
        return projects.map(toProjectResponse);
    }
    /**
      ...
      */
    static async findCarouselItems() {
        const projects = await project_model_1.ProjectModel.find({ isCarousel: true }).lean();
        return projects.map(toProjectResponse);
    }
    /**
      ...
      */
    static async findCarouselItemsSorted() {
        const projects = await project_model_1.ProjectModel.find({ isCarousel: true })
    ...
    */
    ...
    static async delete(id) {
        const result = await project_model_1.ProjectModel.deleteOne({ _id: id });
   ...
   */
    static async findByCategory(category, page = 1, limit = 6) {
